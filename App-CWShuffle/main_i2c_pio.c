 ...
 *  */
 ...
 void shuffle_task(void* unused_arg) {
    ...
    char pabc[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFGHIJKLMNOPQRSTUVWXYZABCDEFG\0";
    char pnumbers[] = "01234567890123456789012345678901234567890123456789678912\0";
    char pan[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZ012345678901234567890123456789\0";
    char paris[] = "PARIS PARIS PARIS PARIS PARIS\0";
    char pve[]  = "VE7BGV\0";
    char pvel[] = "ve7bgv\0";
    char pph[SPTOOSIZE];
                                
    char *phrases[9] = {pfox,pabc,pnumbers,pan,paris,pve,pvel,pph,0x0000};
...
// Beginning of each Shuffle
    while (true) {
        // check to see if there is a new phrase selection waiting from switches         
         uxMessagesWaiting = uxQueueMessagesWaiting(xQadc);
         if(uxMessagesWaiting){
            xQueueReceive(xQadc,&phrase_select, 0); 
 ...
 */
 ...
 void adc_task(void* unused_arg) {
    ...
    uint32_t now = 1;
    uint32_t interval_val  = 0;
   
    while (true) {
        // Measure ADC and add the LED state
        // to the FreeRTOS xQUEUE
        now = adc_read();
        interval_val = now / (4096/INTERVALS);
        show_seven_seg(interval_val);
    
        uxNumberOfQSpaces = uxQueueSpacesAvailable( xQadc );
        if(uxNumberOfQSpaces > 0){
            xStatus = xQueueSendToFront(xQadc,&interval_val, 0);
        }
...
void configure_gpio(void) {
    ...
    // Configure the D24 Pin LED
    gpio_init(D24_PIN);
    gpio_set_dir(D24_PIN, GPIO_OUT);
    gpio_disable_pulls(D24_PIN);  // remove pullup and pulldowns
...
//    blink_pin_forever(pio, 1, offset, 18, 8);  // approx 18 Hz - 
    blink_pin_forever(pio, 1, offset, 18, 600);  // approx 600 Hz - 
//    blink_pin_forever(pio, 2, offset, 7, 3);
//    blink_pin_forever(pio, 3, offset, 6, 4);
