 ...
 * Text selected by value read in from Keyboard (0 to7)
 * and displayed on 7Seg
...
TaskHandle_t kbd_cmd_task_handle = NULL;
...
volatile QueueHandle_t xQkbdcmd = NULL;
...
// Get next String
    while (true) {
        // check to see if there is a new phrase selection waiting from switches         
            xQueuePeek(xQkbdcmd,&phrase_select, 0);
...
//     char startcmd[] = "PRESS SW6\0";  // Prompt phrase
    ...
    // ******************************************************************************************   
		...
		//  Queue will have phrase waiting if SW6 has been pressed Twice.  If SW6 has not been pressed
		//  there will not be a phrase ready yet so continue to send startcmd 
        if (uxQueueMessagesWaiting(xQshuffle)){  // This prompts Shuffle task to provide next phrase
            xStatus = xQueueReceive(xQshuffle,&pph, 0);
            if(!first_flag){  // First Flag will be changed when first phrase is presented.
                first_flag = 1;
                latch_arm = 1;  
...
/* 
 * @brief kbd_cmd_Task 
 * This provides a Keyboard input for selecting the next phrase to shuffle.
 * It watches for an ASCII number between 0 and 7 then CR.  The number will
 * be changed to an integer value and sent to the phrase select action
 * with a Queue.
 * 
 *  */

 void kbd_cmd_task(void* unused_arg) {

    uint8_t now = 1;
    uint32_t command_out;
    char new_char;
       ...
       while(true){
          ...
          xQueueOverwrite(xQkbdcmd, &command_out);
//				show_seven_seg((uint8_t)command_out);

//				printf(" %c %i\n", new_char, command_out);
		}
...
// Timer creates dot length
    ...
    BaseType_t kbd_status = xTaskCreate(kbd_cmd_task,             
                                         "KBD_CMD_TASK", 
                                         256, 
                                         NULL, 
                                         3,     // Task priority
                                         &kbd_cmd_task_handle);
        if (kbd_status != pdPASS) {
            error_state  += 1;
            }
   ...
   // Set up the event queue
    ...
    xQkbdcmd = xQueueCreate(1, sizeof(uint32_t)); 
    if ( xQkbdcmd == NULL ) error_state += 1;
