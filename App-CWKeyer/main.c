 ...
 * Timers are monostable, creating delay between words and text sent
 ...
 *        xQphrase triggers TX of phrase chosen by ADC input value
 */
...
//    uint8_t last_char = 0;
   ...
   while (true) {
        ...
        if (phrase_timer != NULL) xTimerStart(phrase_timer, 0);
        xQueueReceive(xQphrase, &t1_info, portMAX_DELAY); // xQueueReceive empties queue
...
// *********************************************************************************************     
    ...
    while (morse_out != 1) {              // send Morse bits              
        ...
        if (dit_timer != NULL) xTimerStart(dit_timer, 0);  // define dit length 
        xQueueReceive(xQdit, &t1_info, portMAX_DELAY); // xQueueReceive empties queue
        }
 ...
 * @param timer: The triggering timer.
 */
void dit_timer_fired_callback(TimerHandle_t timer) {
    struct op
    {
        uint32_t t_number;
        uint32_t t_state;
 ...
 */
 ...
 void select_phrase_task(void* unused_arg) {  // selects which Text phrase to o/p
    ...
    uint32_t now = 1;
    uint32_t select_val = SENTENCE;
    uint32_t sw1_state  = SENTENCE;
    uint32_t sw2_state  = SENTENCE;
    uint32_t sw3_state  = SENTENCE;
   
    while (true) {
        // Read Switchs  and add the LED state
        // to the FreeRTOS xQUEUE
        sw3_state = gpio_get(SW3);
        sw2_state = gpio_get(SW2) * 2;
        sw1_state = gpio_get(SW1) * 4;
        select_val = sw3_state + sw2_state + sw1_state;
//        now = adc_read();
//        select_val = now / (4096/INTERVALS);
//        show_seven_seg_num(select_val);
        show_seven_seg(select_val);
...
void configure_gpio(void) {
    ...
    // Configure Switch3
    gpio_init(SW3);
    gpio_pull_up(SW3);  // pullup for switches
