 ...
 * 
 * DOT Timer is monostable, creating delay to form DOT length
 * Audio tone created from Clock output on GPIO
 * After Switch 06 is pressed twice to initiate text output
 * Switch 06 operates as a Latch Switch to choose 
 ...
 * System clock pin 27 provides the audio 500 Hz Tone
 * A CD4010  gate combines the audio and CW  signal.
 * 
 * Switches are scanned and debounced by an RTOS task and made
 * available in Queue output.  GPIO Extender PCF8575 manages switches.
 * 
...
/**
   uint32_t lines = 8;                            
    ...
    const char p00[] = "ABOUT CAN GOOD LARGE ON THAN WAS AFTER COULD GOT LAST ONE THAT WE\0";
    const char p01[] = "ALL COUNTRY GREAT LIKE ONLY THE WELL AN DAY HAD LITTLE OR THEIR WENT\0";
    const char p02[] = "AND DAYS HAS LOOK OTHER THEM WERE AM DEBTS HAVE MADE OUR THEN WHAT\0";
    const char p03[] = "ARE DID HE MAKE OUT THERE WHEN AS DO HER MANY OVER THESE WHERE DOLLARS\0";
    const char p04[] = "HERE MAY PEOPLE THEY WHICH BACK DOWN HIM ME PUBLIC THIS WHILE EVEN HIS\0";
    const char p05[] = "MORE SAID THROUGH WHO BECAUSE EVERY I MOST SAW TIME WILL BEEN FIRST IF MUST\0";
    const char p06[] = "SEE TO WITH BEFORE FOR IN MY SHE TODAY WOULD BEING FOUND INTEREST NEW SINCE\0";
    const char p07[] = "TWO YEARS BETWEEN FROM INTO NO SO UNDER YOU BIG GENERAL IS NOT SOME UP YOUR\0";
                                
    const char *phrases[] = {p00,p01,p02,p03,p04,p05,p06,p07};
*/
/** 
    uint32_t lines = 8;
...
/**
    ...
    const char p00[] = "THE THE THE AND AND AND FOR FOR FOR ARE ARE ARE BUT BUT BUT USE USE USE\0";
    const char p01[] = "OF OF TO TO IN IN IT IT IS IS BE BE AS AS AT AT SO SO WE WE\0";
    const char p02[] = "NOT NOT NOT YOU YOU YOU ALL ALL ALL ANY ANY ANY CAN CAN CAN\0";
    const char p03[] = "HE HE BY BY OR OR ON ON DO DO IF IF ME ME MY MY UP UP AN AN\0";
    const char p04[] = "HAD HAD HAD HER HER HER WAS WAS WAS ONE ONE ONE OUR OUR OUR\0";
    const char p05[] = "GO GO NO NO US US AM AM OF OF TO TO IN IN IT IT IS IS BE BE AS AS\0";
    const char p06[] = "OUT OUT OUT DAY DAY DAY GET GET GET HAS HAS HAS HIM HIM HIM\0";
    const char p07[] = "AT AT SO SO WE WE HE HE BY BY OR OR ON ON DO DO IF IF ME ME MY\0";
    const char p08[] = "HIS HIS HIS HOW HOW HOW MAN MAN MAN NEW NEW NEW NOW NOW NOW\0";
    const char p09[] = "OLD OLD OLD SEE SEE SEE TWO TWO TWO WAY WAY WAY WHO WHO WHO\0";
    const char p0A[] = "MY UP UP AN AN GO GO NO NO US US AM AM BOY BOY BOY DID DID DID\0";
    const char p0B[] = "ITS ITS ITS LET LET LET PUT PUT PUT SAY SAY SAY SHE SHE SHE TOO TOO TOO\0";
                                
    const char *phrases[] = {p00,p01,p02,p03,p04,p05,p06,p07,p08,p09,p0A,p0B};
*/

/**
    uint32_t lines = 3;                            

    const char p00[] = "THE AND FOR ARE BUT USE\0";
    const char p01[] = "OF TO IN IT IS BE AS AT SO WE \0";
    const char p02[] = "NOT YOU ALL ANY CAN\0";

    const char *phrases[] = {p00,p01,p02};
*/
       ...
       while (true) {
        ...
        while (i < string_length) {   // step through TX phrase
           ...
           // ******************************************************************************************        
            ...
            if(!latch_buffer) {  // if Toggle LOW, Send CW Slower
...
//                send_CW(0x20); //  Space char after each Char sent to slow
    ...
    // ******************************************************************************************   
        ...
        printf("\n");
            ...
            if(first_flag){
                latch_arm = 1;  
                xQueueOverwrite(xQlatcharm, &first_flag); // ARM latch
             }
 ...
 * the switch can do the change speed job.
 */
