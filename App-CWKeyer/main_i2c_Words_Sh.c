...
TaskHandle_t sw7_latch_task_handle = NULL;
...
void shuffle_phrases_task(void* unused_arg) {
    ...
    uint8_t sw7_buffer = 0;         // initialize Queue input buffer
        ...
        while (i < string_length) {   // step through TX phrase
           ...
           // ******************************************************************************************        
            ...
            if(!latch_buffer) {  // if Toggle LOW, Send CW Slower
                send_CW(0x8); //  Long Space char after each Char sent to slow
//                send_CW(0x10); //  Space char after each Char sent to slow
            }
          // ******************************************************************************************   
 ...
 */
 ...
 void sw7_latch_task(void* unused_arg) {
...
// label Program Screen
    ...
    printf("\x1B[%i;%ir",6,30);  // set window top and bottom lines
...
// Timer creates dot length
    ...
    BaseType_t latch_status = xTaskCreate(sw7_latch_task,             
                                         "SW7_LATCH_TASK", 
                                         256, 
                                         NULL, 
                                         5,     // Task priority
                                         &sw7_latch_task_handle);
