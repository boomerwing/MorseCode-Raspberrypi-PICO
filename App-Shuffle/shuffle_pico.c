 ...
 void sw1_debounce(void* unused_arg) {
    ...
    while (true) {
        ...
        if (count > MINCOUNT) //  switch has stopped bouncing
            {       
                xQueueOverwrite(xQueue1, &sw_state);
//            xQueueSendToFront(xQueue1, &sw_state, 0);
            }  //  End if (count > 5 ) 
 ...
 */
 void sw2_debounce(void* unused_arg) {
    ...
    while (true) {
        ...
        if (count > MINCOUNT) //  switch has stopped bouncing
            {       
                xQueueOverwrite(xQueue2, &sw_state);
//            xQueueSendToFront(xQueue2, &sw_state, 0);
...
//  create random seed with initial Toggle of Switch 3
        ...
        xQueuePeek(xQueue1, &passed_value_buffer,0);
...
int main() {
   ...
   // Set up the event queue
    ...
    if ( xQueue1 == NULL ) error_state += 1;

    xQueue2 = xQueueCreate(1, sizeof(uint32_t)); 
    if ( xQueue2 == NULL ) error_state += 1;
