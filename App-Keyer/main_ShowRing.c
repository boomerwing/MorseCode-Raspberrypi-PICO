...
void ringbuffer_in_task(void* unused_arg) {
    ...
    while(true){
		if(char_count<RING_SIZE-1){
			...
			if(new_char){
				*next++ = new_char;   // put new_char in ring, incr next
				if(next>ring_end){
					next = ring;
				}
			char_count++;
...
/**
 * @brief showRing
 * Display a printout of the 'next' position and 'take' position
 * of the ring buffer
 * text output     .T....X.........
 */
 
void showRing(void) {  //  next and take variables are Global
	char ring_image[RING_SIZE+1];
    char* ring_image_end = ring_image + (RING_SIZE -1) ;
    uint8_t nextPosition;
    uint8_t takePosition;
    int i;
    
    for(i=0;i<RING_SIZE;i++){
		ring_image[i] = '.';
	}
	ring_image[RING_SIZE] = '\0';
	          
	nextPosition = (next - ring);  // Subtract pointers
	takePosition = (take - ring);  // Subtract pointers
	
	ring_image[nextPosition] = 'X';
	ring_image[takePosition] = 'T';
	if(nextPosition == takePosition){
		ring_image[nextPosition] = 'W';
	}
...
void configure_gpio(void) {
    ...
    // Configure GPIOCLOCK0 to create 500 Hz tone on GPIO 21 using System PLL clock source
