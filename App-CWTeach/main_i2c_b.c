...
//            sw_state = sw1_buffer;
        ...
        printf("\x1B[K");  //  clear to end of Line Right
        printf("%s",deck1);    //  print out start string
...
//         uxMessagesWaiting = uxQueueMessagesWaiting(xQshuffle);
//         if(uxMessagesWaiting){
//            for(i=0;i<DECKSIZE; i++) { // purge pph buffer
...
//         }
        ...
        show_seven_seg_i2c(phrase_select);
...
//        printf("\n"); 
        ...
        if (cwp_timer != NULL)xTimerStart(cwp_timer, 0);
         while(!uxQueueMessagesWaiting(xQpause)){
         if(uxQueueMessagesWaiting(xQpause)){
            xStatus = xQueueReceive(xQpause, &t_state, portMAX_DELAY); // xQueueReceive empties queue
            }
        }
//            xStatus = xQueueReceive(xQpause, &tp_info, portMAX_DELAY);
    }  // end of while(true)
} 
...
// *********************************************************************************************     
    ...
    while (morse_out != 1) {              // send Morse bits              
        ...
        if (cwd_timer != NULL) xTimerStart(cwd_timer, 0); 
         if(uxQueueMessagesWaiting(xQdit)){
            xStatus = xQueueReceive(xQdit, &t_state, portMAX_DELAY); // xQueueReceive empties queue
            printf("%u \n",t_state);
            }
             // define dot length 
       }
...
void cwp_timer_fired_callback(TimerHandle_t timer) {
    ...
    if (timer == cwp_timer) ;
        ...
        xQueueOverwrite(xQpause, &t_state);
}
 ...
 */
 void sw1_debounce(void* unused_arg) {
    ...
    while (true) {
        ...
        if(last == now) {
            vTaskDelay(ms_delay400);  // check switch state every 50 ms
        }
