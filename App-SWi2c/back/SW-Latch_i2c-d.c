 ...
 void led2_task(void* unused_arg) {
    ...
    while (true) {
        ...
        if(MsgWaitingkey){ // Look for start Command with Switch 1
            ...
            sw_state = passed_value_buffer;
            if(sw_state == 0) {
                // Timer added as experiment Delays System ON command.
//                if (dot_timer != NULL) xTimerStart(dot_timer, 0);  // define dit length 
                if (dot_timer != NULL) xTimerChangePeriod(dot_timer, dot_timer_period, 0);  // define dit length 
                xQueueReceive(xQdot, &timer_info, portMAX_DELAY); // xQueueReceive empties queue
                printf(" ** System ON ** %u \n", timer_info.t_number);
//                printf(" ** System ON  ** \n");
